...
//#define SENSOR_ALWAYS_REQPORT_DEBUG		// test purpose: always report measurement even speed is zero
...
#define ACCEL_LIS2DE12_DATARATE  				LIS2DE12_REG1_50HZ
#endif
...
#define ACCEL_MOVE_SPEED_MID_G_VALUE				1.3f	//accelerometer value >1.14 or <-1.14
#define ACCEL_MOVE_SPEED_HIGH_G_VALUE				4.0f	//accelerometer value >2or <-2
/* Speed calculation interval (milliseconds). */
#define ACCEL_SPEED_AND_CADENCE_CAL_INTERVAL 		1000
/* Factor for check speed interval = SPEED_AND_CADENCE_MEAS_INTERVAL/ACCEL_SPEED_AND_CADENCE_CAL_FACTOR. */
#define ACCEL_SPEED_AND_CADENCE_CAL_FACTOR 		2
...
#ifndef ACCELEROMETER_DUMP_FIFO
				for(int i=0; i<DEF_WATERMARK_VAL; i++)
				{
					accel_read_reg(LIS2DE12_OUT_X_L, &accel_buff[6*i]);
					accel_read_reg(LIS2DE12_OUT_X_H, &accel_buff[6*i+1]);
					accel_read_reg(LIS2DE12_OUT_Y_L, &accel_buff[6*i+2]);
					accel_read_reg(LIS2DE12_OUT_Y_H, &accel_buff[6*i+3]);
					accel_read_reg(LIS2DE12_OUT_Z_L, &accel_buff[6*i+4]);
					accel_read_reg(LIS2DE12_OUT_Z_H, &accel_buff[6*i+5]);

					//NRF_LOG_INFO("id: %d, OUT_X_H: %x, OUT_Y_H, %x, OUT_Z_H: %x",i, accel_buff[i+1], &accel_buff[i+3], &accel_buff[i+5]);
				}
#endif
			}
			accel_read_reg(LIS2DE12_FIFO_SRC_REG, 	&fifo_src);
...
#ifdef SENSOR_DEBUG_OUTPUT
	NRF_LOG_INFO("LIS2DE12_int2 INT2_SRC: %x, FIFO_SRC: %x ", int_src, fifo_src);
#endif
#ifdef SENSOR_DEBUG_OUTPUT
...
static void application_timers_start(void)
{
    ...
    uint32_t csc_meas_timer_ticks = APP_TIMER_TICKS(SPEED_AND_CADENCE_MEAS_INTERVAL/ACCEL_SPEED_AND_CADENCE_CAL_FACTOR);
...
*	@param config : value t_accel_speed_flag
*/
static void speed_flag_set(uint16_t current_speed)
{
	/* Threshold of square of accelerometer value */
	#define ACCEL_MOVE_SPEED_HIGH_MH				(1904*WHEEL_CIRCUMFERENCE_MM/100) //39984 = 39.9 Km/h  (degree*WHEEL_CIRCUMFERENCE_MM*3600/360/1000)
	#define ACCEL_MOVE_SPEED_MEDHIGH_MH				(1429*WHEEL_CIRCUMFERENCE_MM/100) //30009 = 30.0 Km/h  (degree*WHEEL_CIRCUMFERENCE_MM*3600/360/1000)
	#define ACCEL_MOVE_SPEED_MED_MH					(762*WHEEL_CIRCUMFERENCE_MM/100) //16002 = 16.0 Km/h  (degree*WHEEL_CIRCUMFERENCE_MM*3600/360/1000)
	...
	else if(current_speed > ACCEL_MOVE_SPEED_MED_MH)
	{
		acc_speed_high_flag = ACCE_SPEED_MID;
	}
	else
	{
    	if(((last_average_weighting * last_average_weighting) < ACCEL_MOVE_SPEED_MID_G_VALUE))
    	{
...
static void accel_configuration(void)
{
	...
	/* clear interrupt in case there were something vestigial */
	//uint8_t ui8_temp = 0;
	//accel_read_reg(LIS2DE12_INT1_SRC_REG, 	&ui8_temp);
...
#ifdef ACC_ST16G_ENABLE
/**@brief Function for accel_lis2de12_reset_reg.
 */
static void accel_lis2de12_reset_reg(void)
{
...
static void accel_config_fifo_int(bool enable)
{
	...
	if(enable)
	{
		...
		//accel_write_reg(LIS2DE12_REG5, LIS2DE12_REG5_USE_FIFO |LIS2DE12_REG5_LIR_INT1|LIS2DE12_REG5_LIR_INT2 );
	}
	else
	{
...
static float lowPassExponential(float input, float average, float factor, float limited_factor)
{
	float low_pass_diff = (abs(input - average) >= limited_factor)? limited_factor * (float)(acc_sign(input - average)): input - average;
...
void acc_read_fifodata(void)
{
	...
	for(uint8_t i = 0; i < DEF_WATERMARK_VAL; i++)
	{
		...
		//ax = (float)(x_sample2[i]*ACCEL_SENSITIVITY_MG_CONFIG)/(float)(16 * 1000);
		ay = (float)(y_sample2[i]*ACCEL_SENSITIVITY_MG_CONFIG)/(float)(16 * 1000);
		az = (float)(z_sample2[i]*ACCEL_SENSITIVITY_MG_CONFIG)/(float)(16 * 1000);
#endif
...
static void acc_step_mag_update(float mag_update_value)
{
	...
	#define MAX_FILTER_WINDOW_LOW_SPEED	   		0.16f
...
#else
	Z_offset = (Zout_12_bit - ACCEL_SENSITIVITY_MG_CONFIG *1000) / 2 * (-1);
#endif
...
ret_code_t accel_csc_measurement(ble_cscs_meas_t * p_measurement)
{
    ...
    static uint16_t report_counter = 0;	// counter for report csc_measurement or not. if it do not need to report means u
...
#ifndef SENSOR_ALWAYS_REQPORT_DEBUG
    else if((ui32_last_lap == ui32_total_step) && !acc_meas_report_flag)
    {

        if(mid_speed_flag_get())
    	{
        	speed_flag_set(0); 	// rest speed flag
    	}
#ifdef SENSOR_DEBUG_OUTPUT
		NRF_LOG_INFO("accel_csc_measurement(): lap without change, skip report");
	...
	/* check need to report or not*/
	report_counter ++;
	if(report_counter % (SPEED_AND_CADENCE_MEAS_INTERVAL/ACCEL_SPEED_AND_CADENCE_CAL_INTERVAL) != 0)
	{
		return NRF_ERROR_INVALID_STATE;		// no need to report csc_measurement
	}
